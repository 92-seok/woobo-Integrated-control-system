...
try {
    ...
    // CD_DIST_OBSV
    $formCdDistObsv = $form['cdDistObsv'] ?? '';
    $res['logName'] = $formCdDistObsv;
    $res['logBefore'] = '';
    $res['logAfter'] = '';

    $obsvArray = explode(',', $formCdDistObsv);

    // DisCode
    $disCode = $form['disCode'] ?? '';

    foreach ($obsvArray as $cdDistObsv) {
        $eqp_vo = $eqp_dao->SINGLE("CD_DIST_OBSV = '{$cdDistObsv}'");
        if ($eqp_vo === false) {
            throw new Exception('Dont search cdDistObsv from DB', 404);
        }

        // 로거모델
        $eqpModel[$cdDistObsv] = $eqp_vo->ConnModel;

        // LOG 처리
        if ($type == 'update' || $type == 'end') {
            $dis_vo = $dis_dao->SINGLE("CD_DIST_OBSV = '{$cdDistObsv}'");
            if ($dis_vo !== false) {
    ...
    foreach ($obsvArray as $cdDistObsv) {
        ...
        if ($type == 'end') {
            ...
            $dis_dao->Update("Exp_YN = 'N'", "CD_DIST_OBSV = '{$cdDistObsv}'");
            ...
            if ($dis_vo === false) {
                throw new Exception("Dont search row from DB ({$disCode})", 404);
            }
            ...
            foreach ($disCode as $code) {
                ...
                if ($dis_vo === false) {
                    throw new Exception("Dont search row from DB ({$code})", 404);
                }
        ...
        if ($type == 'insert' || $type == 'update') {
            ...
            $viewType = '';
            // 로거모델
            $connModel = $eqpModel[$cdDistObsv];
            if ($connModel === 'EWDISPLAY_LAN') {
                $viewType = 'text';
            } elseif ($connModel === 'EWDPL_LAN') {
                $viewType = 'image';
            } else {
                throw new Exception('Unknown Logger Model: ' . $connModel, 404);
            }
            switch ($viewType) {
                case 'text':
                    ...
                    $dis_dao->imageResize("../../{$dis_vo->ViewImg}", 0.5, 0.5);
                    break;
        ...
        if ($type == 'insert') {
            ...
            $res['logEvent'] = 'Group Scenario Add';
        } elseif ($type == 'update') {
