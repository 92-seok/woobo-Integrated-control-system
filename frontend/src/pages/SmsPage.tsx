...
import { RecipientTable } from '@/components/sms/RecipientTable';
...
export function SmsPage() {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-background">
      ...
      <div
        className={`fixed inset-y-0 left-0 z-50 transform transition-transform duration-300 md:hidden ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}
      >
        ...
        <header className="flex items-center justify-between border-b border-border bg-card px-6 py-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">문자 발송 및 연락처 관리</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <Tabs defaultValue="send" className="flex flex-col gap-6">
            <TabsList className="inline-flex w-fit rounded-lg bg-muted p-1">
              <TabsTrigger
                value="send"
                className="gap-2 rounded-md px-4 py-2 text-sm data-[state=active]:bg-card data-[state=active]:text-foreground data-[state=active]:shadow-sm"
              >
